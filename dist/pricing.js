...
exports.getPricingBreakdown = getPricingBreakdown;
exports.isInstantPriceAvailable = isInstantPriceAvailable;
// ============================================
// CONSTANTS
// ============================================
...
const PRINT_METHOD_COSTS = {
    'screen-print': (colorCount) => 1.50 * colorCount,
    'dtg': 3.50,
 ...
 */
function getPrintMethodCost(printMethod, colorCount) {
 ...
 */
function getColorUpcharge(colorCount) {
    if (colorCount <= 2) {
        return 0;
    }
 ...
 */
function getPrimaryPlacementMultiplier(placements) {
 ...
 */
function getAdditionalPlacementCosts(placements) {
 ...
 */
function calculateUnitPrice(product, quantity, printMethod, placements, colorCount) {
    // Validate inputs
    if (quantity < 1) {
 ...
 */
function calculateTotalPrice(unitPrice, quantity, sizes) {
    // Validate size distribution if provided
    if (sizes) {
        const totalFromSizes = Object.values(sizes).reduce((sum, count) => sum + (count || 0), 0);
        if (totalFromSizes !== quantity) {
            throw new Error(`Size distribution total (${totalFromSizes}) does not match quantity (${quantity})`);
        }
    }
 ...
 */
function getPricingBreakdown(product, quantity, printMethod, placements, colorCount) {
    // Validate inputs
    if (quantity < 1) {
        throw new Error('Quantity must be at least 1');
    }
 ...
 */
function isInstantPriceAvailable(product, printMethod) {
